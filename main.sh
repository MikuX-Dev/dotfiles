...
#####################################################################################
welcome() {
  print_color "${CYAN}" "Welcome to the system setup script!"

  while true; do
    choice=$(prompt "What would you like to do?")
    print_color "${YELLOW}" "1) Install Arch Linux"
    print_color "${YELLOW}" "2) Set up dotfiles"
    print_color "${YELLOW}" "3) Configure Git"
    print_color "${YELLOW}" "4) Set up Git with SSH"
    print_color "${YELLOW}" "5) Exit"
    case $choice in
    1)
      # Install Arch Linux
      print_color "${YELLOW}" "Installing Arch Linux..."
      archinstall
      ;;
    2)
      # Set up dotfiles
    ...
    3)
      # Configure Git
      print_color "${YELLOW}" "Configuring Git..."
      setupgit
      ;;
    4)
      # Set up Git with SSH
      print_color "${YELLOW}" "Setting up Git with SSH..."
      setupgitssh
      ;;
    5)
      print_color "${RED}" "Exiting script."
      exit 1
      ;;
    *)
...
EOF
  ...
  print_color "${CYAN}" "Installing package using paru..."
  printf "\n"
  mkdir -p "$SKEL"/aur-pkg/pikaur
  git clone https://aur.archlinux.org/pikaur.git "$SKEL"/aur-pkg/pikaur
  cd "$SKEL"/aur-pkg/pikaur || exit
  makepkg -csi --skippgpcheck --noconfirm --needed
  cd - || exit
...
dots() {
  ...
  sleep 3s
  printf "\n"
  print_color "${BLUE}" "Installation starts now..."
  sleep 3s
  clear
